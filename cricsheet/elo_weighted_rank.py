...
def zscore_sample(x: pd.Series) -> pd.Series:
    ...
    if pd.isna(sd) or sd == 0: return pd.Series(0.0, index=x.index)
    z = (x - mu) / sd
...
def build_feature_matrix(df: pd.DataFrame):
    ...
    if not feats: raise RuntimeError("No weighted features found with weight > 0.")
...
def compute_weighted_perf(X: pd.DataFrame) -> pd.Series:
    ...
    if denom == 0: raise RuntimeError("Sum of absolute weights is zero.")
...
def importance_scale(row: pd.Series) -> float:
    ...
    stage_num = np.tanh(stage_num/5.0); venue_num = np.tanh(venue_num/20.0)
    win_type_num = np.tanh(win_type_num/5.0); win_margin_num = np.tanh(win_margin_num/50.0)
    ...
    mult = 1.0 + 0.25 * np.tanh(s / 20.0) + 0.25  # ~[0.8, 1.5] around 1.25
    return float(mult)
...
def main():
    ...
    for idx in order_idx:
        ...
        rows.append({
            "Player": p, "Match_ID": row[match_col], "Date": row[date_col],
            "WeightedPerf": float(weighted_perf.iloc[idx]), "ObservedScore": s,
            "Elo_Before": rating_before, "Elo_Expected": E, "K_used": float(K),
            "Elo_After": rating_after, "Opponent": row.get("opponent", np.nan),
            "Venue": row.get("venue", np.nan), "Event_Stage": row.get("event_stage", np.nan),
            "Win_Type": row.get("win_type", np.nan), "Win_Margin": row.get("win_margin", np.nan),
            "Runs_Scored": row.get("runs_scored", np.nan), "Balls_Faced": row.get("balls_faced", np.nan),
            "Fours": row.get("fours", np.nan), "Sixes": row.get("sixes", np.nan),
    ...
    print(f"Saved: player_elo_ratings.csv, match_scores.csv (from {INPUT_FILE})")
